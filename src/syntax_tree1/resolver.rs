...
pub trait NodeResolver: Sync {
    fn handle<'i>(&self, pair: Pair<'i, Rule>) -> Result<Node<'i>, Error>;
    fn handle_pratt<'i>(&self, pair: PrattPair<'i>) -> Result<Node<'i>, Error>;
...
pub fn handle_pair<'i>(pair: Pair<'i, Rule>) -> Result<Node<'i>, Error> {
    NODES
        .get(&pair.as_rule())
...
pub fn handle_pratt<'i>(pair: PrattPair<'i>) -> Result<Node<'i>, Error> {
    NODES
        .get(&pair.as_rule())
        .or_error(ErrorDetails::Internal {
            msg: format!("No handler for rule {:?}", pair.as_rule()),
