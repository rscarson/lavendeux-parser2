...
define_prattnode!(
    ...
    new = (input) {
        ...
        let decorator_pair = children.next().unwrap().first_pair();
        let decorator = decorator_pair.into_inner().next().unwrap().as_str().to_string();
    ...
    into_owned = (this) {
        Self {
            ...
            decorator: this.decorator.clone(),
...
impl<'i> ConstantValue<'i> {
    pub fn new(value: Value, token: crate::Token<'i>) -> Node<'i> {
