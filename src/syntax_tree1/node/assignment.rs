...
impl<'i> AssignmentTarget<'i> {
    pub fn into_owned(&self) -> AssignmentTarget<'static> {
        match self {
            Self::Identifier(id) => Self::Identifier(id.clone()),
            Self::Index(base, indices) => Self::Index(
                base.clone(),
                indices
                    .iter()
                    .map(|i| i.clone().map(|i| i.into_owned()))
                    .collect(),
            ),
            Self::Destructure(ids) => Self::Destructure(ids.clone()),
        }
    }

    /// Consumes a pair, returning an `AssignmentTarget` if it's valid
    pub fn from_pair(input: PrattPair<'i>) -> Result<Self, Error> {
        match input.as_rule() {
