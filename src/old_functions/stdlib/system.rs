use crate::{
    ...
    std_functions::Function, Lavendeux, State,
};
...
pub fn register_all(map: &mut HashMap<String, Function>) {
    static_function!(
        ...
        handler = |_state: &mut State, arguments, token, _| {
            ...
            oops!(Custom { message: message }, token.clone())
        }
    );
    ...
    static_function!(
        registry = map,
        name = "would_err",
        description = "Throws an error if the given expression does not throw an error",
        category = "system",
        ...
        handler = |state: &mut State, arguments, token, _| {
            ...
            match res {
                Ok(_) => {
                    let message = format!("Expected an error from expression: {}", expression);
                    oops!(Custom { message: message }, token.clone())
                }
                Err(e) => Ok(Value::from(e.to_string())),
            }
        }
    );
    ...
    static_function!(
        ...
        handler = |_: &mut State, arguments, token, _| {
            let expression = get_argument!("expression", arguments).to_string();
            crate::extensions::ExtensionController::exec(&expression).with_context(token)
        }
    );
    ...
    static_function!(
        ...
        handler = |_: &mut State, arguments, token, _| {
            ...
            crate::extensions::ExtensionController::with(|controller| {
                let extension = controller.register(&filename).with_context(token)?;
    ...
    static_function!(
        ...
        handler = |state: &mut State, arguments, token, _| {
            let filename = get_argument!("filename", arguments).to_string();
            let script = std::fs::read_to_string(filename).with_context(token)?;
