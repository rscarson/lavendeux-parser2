use crate::{
    error::WrapExternalError, get_argument, oops, required_argument, static_decorator,
    static_function, std_functions::Function, State,
};
use polyvalue::{
...
pub fn register_all(map: &mut HashMap<String, Function>) {
    static_decorator!(
        ...
        handler = &|input, token| {
            let mut input = *input.as_a::<I64>().with_context(token)?.inner();
            if input > 3999 {
                return oops!(Overflow, token.clone());
            }
    ...
    static_decorator!(
        ...
        expected_type = ValueType::Numeric,
        handler = &|input, token| {
            let input = *input.as_a::<Float>().with_context(token)?.inner();
