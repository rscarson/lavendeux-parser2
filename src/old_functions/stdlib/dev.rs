use crate::{
    error::WrapExternalError, get_argument, get_optional_argument, oops, optional_argument,
    required_argument, static_function, std_functions::Function, Error, State,
};
use polyvalue::{
    types::{Array, Str, I64},
    ...
    static_function!(
        ...
        handler = |_: &mut State, arguments, token, _| {
            ...
            let file = get_argument!("file", arguments)
                ...
                .inner()
                .clone();
            let file = File::open(file).with_context(token)?;

            let lines = std::io::BufReader::new(file)
                .lines()
                .map(|f| Ok::<Value, Error>(Value::from(f.with_context(token)?)))
        ...
        handler = |_: &mut State, arguments, token, _| {
            ...
            match (m, n) {
                ...
                (Some(m), None)|(None, Some(m)) => {
                    if m == 0 {
                        return oops!(Range { input: "0".to_string()}, token.clone());
                    }
                    Ok(Value::from(rand::thread_rng().gen_range(0..(m+1))))
                },
                (Some(m), Some(n)) => {
                    if m >= n {
                        return oops!(Range { input: format!("{}..{}", m, n)}, token.clone());
                    }
